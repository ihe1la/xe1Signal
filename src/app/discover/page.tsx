...
export default function DiscoverPage() {
  ...
  React.useEffect(() => {
    ...
    let active = true;
    setReady(false);
    fetch(`/api/signals?limit=24&scope=public&authorId=${encodeURIComponent(session.user.id)}`).then((response) => response.ok ? response.json() : null).then((data) => {
  ...
  return (
    <AppLayout>
      ...
      <div className="mb-5 flex items-center gap-2 overflow-x-auto pb-1 font-mono text-[9px] text-zinc-600">
        ...
        {types.map((item) => <button key={item} onClick={() => setType(item)} className={`shrink-0 rounded-full border px-3 py-1.5 transition ${type === item ? "border-violet-400/45 bg-violet-500/15 text-violet-200" : "border-white/[.055] hover:text-zinc-300"}`}>{item === "AUDIO" ? "VOICE" : item}</button>)}
        <select value={sort} onChange={(event) => setSort(event.target.value)} className="ml-auto rounded-full border border-white/[.055] bg-[#0b0c10] px-3 py-1.5 outline-none"><option value="latest">LATEST</option><option value="strongest">STRONGEST</option><option value="saved">MOST SAVED</option></select>
      </div>
      {!ready ? <LoadingGrid /> : signals.length ? <div className="grid grid-cols-1 items-stretch gap-5 sm:grid-cols-2 xl:grid-cols-3">{signals.map((signal) => <SignalCard key={signal.id} signal={signal} />)}</div> : <div className="rounded-xl border border-dashed border-white/10 px-6 py-20 text-center"><p className="font-mono text-sm text-zinc-300">No signals on this frequency</p><button onClick={() => setType("ALL")} className="mt-3 font-mono text-xs text-violet-300">Clear filter</button></div>}
    </AppLayout>
  );
}
