  ...
  return (
    <AppLayout>
      ...
      {tab === "Collections" ? (
        ...
        !ready ? <div className="h-48 animate-pulse rounded-xl border border-white/[.05] bg-white/[.02]"/> : items.length ? <div className={list ? "grid gap-3" : "grid grid-cols-1 items-stretch gap-5 sm:grid-cols-2 xl:grid-cols-3"}>{items.map((item) => <SignalCard key={item.id} signal={item} variant={list ? "compact" : "default"}/>)}</div> : <EmptyLibrary text={`Nothing in ${tab.toLowerCase()} yet.`} />
      )}
    </AppLayout>
  );
}
