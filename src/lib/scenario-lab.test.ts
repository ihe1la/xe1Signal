...
describe("Scenario Lab access", () => { it("requires both the flag and hela", () => { expect(canAccessScenarioLab(true, "hela")).toBe(true); expect(canAccessScenarioLab(false, "hela")).toBe(false); expect(canAccessScenarioLab(true, "other")).toBe(false); }); });
