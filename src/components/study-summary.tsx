...
export function StudyUnavailable({ reason = "Study data is unavailable right now." }: { reason?: string }) {
  return (
    <section className="rounded-lg border border-white/[.07] bg-white/[.015] px-5 py-12 text-center">
      ...
      <a href={TRACKER_URL} target="_blank" rel="noreferrer" className="mt-5 inline-flex font-sans text-xs text-violet-300 hover:text-violet-200">
        Open tracker ↗
      </a>
    </section>
  );
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/[.06] pb-4">
        <div>
          <p className="font-sans text-sm font-medium text-zinc-200">@{summary.username}</p>
        ...
        <p className="font-sans text-[10px] text-zinc-500">Updated {formatUpdatedAt(summary.updatedAt)} · refreshes every 3h</p>
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.1fr)_minmax(300px,.9fr)]">
        <section className="rounded-lg border border-white/[.07] bg-white/[.015] p-5">
          <div className="flex items-center justify-between gap-3">
            <h2 className="font-sans text-sm font-medium text-zinc-200">Insights</h2>
            <span className="font-mono text-[10px] text-zinc-600">today · week</span>
          </div>
          ...
          <div className="mt-5">
            ...
            {summary.todayLabels.length ? (
              <div className="mt-3 space-y-3">
                {summary.todayLabels.map((item) => (
                  <div key={item.name}>
                    <div className="mb-1 flex items-center justify-between gap-3 font-sans text-xs">
            ...
            ) : <p className="mt-3 font-sans text-xs text-zinc-500">No labeled focus logged today.</p>}
          </div>

          <div className="mt-7">
            <p className="font-mono text-[10px] uppercase tracking-[.14em] text-zinc-500">Last 7 days</p>
            ...
            ) : <p className="mt-3 font-sans text-xs text-zinc-500">No weekly insight data yet.</p>}
          </div>
        </section>

        <section className="rounded-lg border border-white/[.07] bg-white/[.015] p-5">
          <div className="flex items-center justify-between gap-3">
            <h2 className="font-sans text-sm font-medium text-zinc-200">Hall of Fame</h2>
            <span className="font-mono text-[10px] text-zinc-600">this week</span>
          </div>
          {summary.hallOfFame.length ? (
            <div className="mt-4 space-y-1">
              {summary.hallOfFame.map((entry) => (
                <div key={`${entry.rank}-${entry.username}`} className={`flex items-center gap-3 rounded-md px-3 py-2.5 font-sans text-xs ${entry.username.toLowerCase() === summary.username.toLowerCase() ? "bg-violet-400/[.08] text-violet-200" : "text-zinc-400"}`}>
      ...
      {summary.level > 0 && summary.xpToNext > 0 && <div className="rounded-lg border border-white/[.07] bg-white/[.015] px-5 py-4"><div className="flex items-center justify-between gap-3 font-sans text-xs text-zinc-500"><span>Next level</span><span className="tabular-nums">{formatNumber(summary.xpIntoLevel)} / {formatNumber(summary.xpIntoLevel + summary.xpToNext)} XP</span></div><div className="mt-2 h-1 overflow-hidden rounded-full bg-white/[.05]"><div className="h-full rounded-full bg-violet-400/60" style={{ width: `${Math.min(100, Math.max(0, progress))}%` }} /></div></div>}
    </div>
  );
}
