...
export function RemoteAppFrame({
  ...
  heightClassName = "h-[calc(100dvh-12.75rem)] min-h-[420px] lg:h-[calc(100dvh-10rem)] lg:min-h-[560px]",
}: RemoteAppFrameProps) {
  ...
  if (mode === "browser-session") {
    return (
      <section className={`flex flex-col gap-4 ${heightClassName}`}>
        <div className="flex shrink-0 items-center justify-between gap-3">
          <h2 className="font-mono text-lg tracking-tight text-zinc-100 sm:text-xl">{title}</h2>
          {showOriginalLink && (
            <a href={url} target="_blank" rel="noreferrer" className="font-mono text-[10px] text-zinc-500 transition hover:text-zinc-200">
              Open original ↗
            </a>
          )}
        </div>
  ...
  return (
    <section className={`flex flex-col gap-4 ${heightClassName}`}>
      <div className="flex shrink-0 items-center justify-between gap-3">
        <div>
          <h2 className="font-mono text-lg tracking-tight text-zinc-100 sm:text-xl">{title}</h2>
          <p className="mt-1 font-mono text-[10px] text-zinc-600">{url.replace(/^https?:\/\//, "")}</p>
        </div>
        <div className="flex items-center gap-1.5">
          ...
          {showOriginalLink && (
            <a href={url} target="_blank" rel="noreferrer" className="ml-1 font-mono text-[10px] text-zinc-500 transition hover:text-zinc-200">
              Open original ↗
            </a>
          )}
        </div>
      ...
      <div className="relative min-h-0 flex-1 overflow-hidden rounded-[10px] border border-white/[.07] bg-[#0b0c10]">
        {loadState === "loading" && (
          <div className="pointer-events-none absolute left-3 top-3 z-10 rounded-md border border-white/[.06] bg-[#0b0c10]/90 px-2 py-1 font-mono text-[9px] text-zinc-600" aria-live="polite">
            Loading {remoteName}...
          </div>
        )}
        {loadState === "error" ? (
          <div className="grid h-full place-items-center px-6 text-center">
            <div>
              ...
              <a href={url} target="_blank" rel="noreferrer" className="mt-3 font-mono text-[10px] text-violet-300 hover:text-violet-200">
                Open {remoteName} ↗
              </a>
            </div>
          </div>
