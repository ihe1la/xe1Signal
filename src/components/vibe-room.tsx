...
function QueueRow({ item, active, onSelect, onRemove }: { item: VibeQueuePayload; active: boolean; onSelect: () => void; onRemove: () => void }) {
  ...
  return <li className={`flex items-center gap-3 px-5 py-4 transition ${active ? "bg-violet-400/[.06]" : ""}`}>
    ...
    {playable && !active ? <button type="button" onClick={onSelect} className="rounded-md p-2 text-zinc-600 transition hover:bg-white/[.05] hover:text-violet-200" aria-label={`Play ${item.title}`}><Play className="h-3.5 w-3.5 fill-current" /></button> : null}
