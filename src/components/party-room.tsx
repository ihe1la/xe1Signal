...
import type { PartyQueuePayload, PartySnapshot } from "@/lib/party-server";

export function PartyRoom() {
  const [snapshot, setSnapshot] = React.useState<PartySnapshot | null>(null);
  ...
  const controlRef = React.useRef<(action: PartyAction, itemId?: string) => Promise<void>>(() => Promise.resolve());
  const nowPlayingRef = React.useRef<PartyQueuePayload | null>(null);
  ...
  const loadSnapshot = React.useCallback(async () => {
    try {
      ...
      if (!response.ok) throw new Error(data?.error || "Party state is unavailable");
      setSnapshot(data);
      setError(null);
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "Party state is unavailable");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  React.useEffect(() => {
    ...
    void load();
    const source = new EventSource("/api/party/events");
    source.addEventListener("party", () => { void load(); });
    source.onerror = () => { source.close(); };
    const poll = window.setInterval(() => { void load(); }, 3_000);
    return () => { active = false; source.close(); window.clearInterval(poll); };
  }, [loadSnapshot]);

  const sendControl = React.useCallback(async (action: PartyAction, itemId?: string) => {
    const response = await fetch("/api/party/control", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ action, ...(action === "clear" ? {} : { itemId }) }),
    });
    const data = await response.json().catch(() => null);
    if (!response.ok) throw new Error(data?.error || "Party control failed");
    setSnapshot(data);
    setError(null);
  }, []);
  ...
  React.useEffect(() => {
    ...
    if (!item?.fileUrl) {
      if (playerRef.current.current?.id.startsWith("party:")) playerRef.current.stop();
      return;
    }
    const track = {
      id: `party:${item.id}`,
  ...
  async function removeItem(itemId: string) {
    try {
      const response = await fetch(`/api/party/queue/${encodeURIComponent(itemId)}`, { method: "DELETE" });
      const data = await response.json().catch(() => null);
  ...
  return <div className="mx-auto max-w-5xl">
    <header className="mb-8 flex flex-col gap-4 border-b border-white/[.06] pb-7 sm:flex-row sm:items-end sm:justify-between">
      <div>
        ...
        <h1 className="font-mono text-[30px] leading-tight tracking-tight text-zinc-100 sm:text-[36px]">Party, together.</h1>
    ...
    <form onSubmit={queueUrl} className="mb-6 rounded-2xl border border-white/[.08] bg-white/[.025] p-3 shadow-2xl shadow-black/10 sm:flex sm:items-center sm:gap-3">
      ...
      <button type="submit" disabled={submitting || !url.trim()} className="mt-2 flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-violet-400/[.16] px-5 font-mono text-[10px] text-violet-200 transition hover:bg-violet-400/[.24] disabled:cursor-not-allowed disabled:opacity-50 sm:mt-0 sm:w-auto">{submitting ? <LoaderCircle className="h-3.5 w-3.5 animate-spin" /> : <Radio className="h-3.5 w-3.5" />} {submitting ? "Preparing audio" : "Add to party"}</button>
    </form>
    ...
    {loading ? <LoadingParty /> : <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(300px,380px)]">
      <section className="rounded-2xl border border-white/[.07] bg-white/[.018]" aria-label="Party queue">
...
type PartyAction = "play" | "pause" | "skip" | "clear";

function QueueRow({ item, active, onRemove }: { item: PartyQueuePayload; active: boolean; onRemove: () => void }) {
