...
import { demoFrequencies } from "@/lib/demo-data";
    ...
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-[272px] flex-col border-r border-white/[0.055] bg-[#08090d] lg:flex">
      ...
      <div className="scrollbar-thin flex-1 overflow-y-auto px-7 py-9">
        ...
        <div className="space-y-1">
          {demoFrequencies.slice(0, 7).map((frequency) => <Link href={`/frequencies/${frequency.id}`} key={frequency.id} className="flex items-center gap-3 rounded-lg px-3 py-2.5 font-mono text-[12px] text-zinc-500 transition hover:bg-white/[.025] hover:text-zinc-300"><span className="h-2 w-2 rounded-full" style={{ background: frequency.color }} /><span className="truncate">{frequency.name}</span></Link>)}
        </div>
      </div>
      <div className="mx-7 mb-7 flex items-center gap-3 border-t border-white/[.06] pt-7"><Link href={`/profile/${username}`} className="flex min-w-0 flex-1 items-center gap-3"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-md border border-white/10 bg-white/[.035]"><CircleUserRound className="h-5 w-5 text-zinc-500" /></span><span className="min-w-0"><span className="block truncate font-mono text-xs text-zinc-200">{username}</span><span className="mt-1 flex gap-1">{[1,2,3,4,5,6].map((bar) => <i key={bar} className={cn("block w-[3px]", bar < 5 ? "bg-violet-400" : "bg-zinc-800")} style={{height: `${3 + bar}px`}} />)}</span></span></Link><Link href="/settings" aria-label="Settings" className="rounded-md p-2 text-zinc-600 hover:bg-white/5 hover:text-zinc-300"><Settings className="h-4 w-4"/></Link></div>
    </aside>
  );
}
