...
export function RightSidebar() {
  ...
  return (
    <aside className="fixed bottom-0 right-0 top-20 z-20 hidden w-[340px] border-l border-white/[.055] bg-[#08090d] 2xl:flex">
      <div className="flex h-full w-full flex-col overflow-hidden bg-gradient-to-br from-white/[.025] to-transparent">
        <section className="px-6 pb-5 pt-6">
          {avatarUrl ? <img src={avatarUrl} alt={username} className="mb-3 h-[68px] w-[68px] rounded-full border border-white/10 bg-black object-cover" /> : <span className="mb-3 grid h-[68px] w-[68px] place-items-center rounded-full border border-white/10 bg-white/[.04] font-mono text-2xl text-zinc-400">{username.slice(0, 1).toUpperCase()}</span>}
          <p className="font-mono text-[15px] text-zinc-100">{name}</p>
          <p className="mt-1 font-mono text-[9px] text-zinc-600">@{username}</p>
          <div className="mt-2.5 flex items-center gap-3 font-mono text-[11px] text-zinc-500"><span>signal strength</span><StrengthBars value={data.profile?.strength || 0} /></div>
        </section>
        ...
        <Link href={`/profile/${username}`} className="mt-auto flex items-center gap-2 px-6 py-5 font-mono text-[9px] uppercase tracking-wider text-zinc-500 hover:text-zinc-200">View profile <ArrowRight className="h-3 w-3" /></Link>
      </div>
    </aside>
  );
}
...
function ContextSection({ label, children }: { label: string; children: React.ReactNode }) {
  return <section className="border-t border-white/[.055] px-5 py-4"><h3 className="mb-3 font-mono text-[9px] uppercase tracking-[.14em] text-zinc-500">{label}</h3>{children}</section>;
}
