...
import { Archive, Compass, Headphones, Plus, Radio, UserRound } from "lucide-react";
import { cn } from "@/lib/utils";

export function MobileNav() {
  const pathname = usePathname();
  const { data: session } = useSession();
  const items = [["/discover", Compass, "Discover"], ["/frequencies", Radio, "Frequencies"], ["/party", Headphones, "Party"], ["/signals/new", Plus, "New"], ["/archive", Archive, "Archive"], [`/profile/${session?.user?.username || "hela"}`, UserRound, "Profile"]] as const;
